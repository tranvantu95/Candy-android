...
public class MyDatePickerDialog extends DatePickerDialog {
    ...
    public MyDatePickerDialog(Context context, int themeResId, OnDateSetListener callBack, int year, int monthOfYear, int dayOfMonth) {
        super(context, themeResId, callBack, year, monthOfYear, dayOfMonth);
        fixDialogError(context,year,monthOfYear,dayOfMonth);
    }
    ...
    private void fixDialogError(Context context, int year, int monthOfYear, int dayOfMonth){
    ...
    public void onDateChanged(DatePicker view, int year, int month, int day) {
        ...
        String input = String.format("%s/%s/%s", day, month + 1, year);
        DateTimeFormatter formatter = DateTimeFormat.forPattern("dd/MM/YYYY");
        LocalDate localDate = formatter.parseLocalDate(input);
        int dayOfWeek = localDate.getDayOfWeek();
        setTitle(String.format("%s年%s月%s日(%s)", year, month + 1, day, Helpers.getDayOfWeek(dayOfWeek)));
    }
}