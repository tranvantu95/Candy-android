package com.candy.android.custom.image_crop.image_crop;
...
import com.candy.android.custom.image_crop.image_crop.animation.SimpleValueAnimator;
import com.candy.android.custom.image_crop.image_crop.animation.SimpleValueAnimatorListener;
import com.candy.android.custom.image_crop.image_crop.animation.ValueAnimatorV14;
import com.candy.android.custom.image_crop.image_crop.animation.ValueAnimatorV8;
import com.candy.android.custom.image_crop.image_crop.callback.CropCallback;
import com.candy.android.custom.image_crop.image_crop.callback.ErrorCallback;
import com.candy.android.custom.image_crop.image_crop.callback.LoadCallback;
import com.candy.android.custom.image_crop.image_crop.callback.SaveCallback;
import com.candy.android.custom.image_crop.image_crop.util.Utils;
...
public class CropImageView extends ImageView {
     ...
     *
     */
    public static final int BITMAP_MAX_W_x_H = 1024 * 1024 / 2;
    ...
    private void setupLayout(int viewW, int viewH) {
        ...
        {//#my custom
            float margin = Math.min(mImgWidth, mImgHeight) / 12;
            mFrameRect.left += margin;
            mFrameRect.top += margin;
            mFrameRect.right -= margin;
            mFrameRect.bottom -= margin;
        }
        mIsInitialized = true;
        invalidate();
    }
    ...
    private void postErrorOnMainThread(final ErrorCallback callback) {
    ...
    private void saveToFile(Bitmap bitmap, final Uri uri) {
        ...
        try {
            ...
            if (outputStream != null) {
                bitmap = resizeBitmapToLessThanOneMb(bitmap);
     ...
     * @param cropCallback ErrorCallback for cropping the image
     * @param saveCallback ErrorCallback for saving the image
     */
    ...
    private Bitmap resizeBitmapToLessThanOneMb(Bitmap src) {
        int sourceW = src.getWidth();
        int sourceH = src.getHeight();
        if (sourceH * sourceW <= BITMAP_MAX_W_x_H) {
            Log.e(TAG, "Size ok");
            return src;
        }
        float srcRatio = Math.round((float) sourceW / (float) sourceH);
        double newW = Math.sqrt(BITMAP_MAX_W_x_H / srcRatio);
        double newH = newW * srcRatio;
